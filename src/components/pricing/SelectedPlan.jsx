...
// --- IMPORT ASSETS ---
// Using the main icons as placeholders for the service icons
...
const SelectedPlan = () => {
  ...
  const managementServices = [
    { id: 'm1', name: "Flux (Social media)", icon: iconVector, checked: true },
    { id: 'm2', name: "Drift (Email marketing)", icon: iconVector, checked: false },
    { id: 'm3', name: "Atlas (Virtual assistant)", icon: iconVector, checked: false },
    { id: 'm4', name: "Summit (Customer success)", icon: iconVector, checked: false },
    { id: 'm5', name: "Signal (Copywriting)", icon: iconVector, checked: false },
  ];

  const directorServices = [
    { id: 'd1', name: "Flux (Social media)", icon: iconHorizon, checked: false },
    { id: 'd2', name: "Drift (Email marketing)", icon: iconHorizon, checked: false },
    { id: 'd3', name: "Atlas (Virtual assistant)", icon: iconHorizon, checked: false },
    { id: 'd4', name: "Summit (Customer success)", icon: iconHorizon, checked: false },
    { id: 'd5', name: "Signal (Copywriting)", icon: iconHorizon, checked: false },
  ];
  ...
  return (
    // Added pt-28 to clear the navbar + 20px margin
              ...
              {managementServices.map((service) => (
                <div key={service.id} className="flex items-center gap-4 group cursor-pointer">
                  ...
                  <div className="w-10 h-10 flex-shrink-0">
                    ...
                    {/* Note: CSS filter used above to shift pink icon to teal if you use placeholder. Remove filter if real icon used. */}
                  </div>
                  
                  <span className={`text-sm font-light transition-colors ${service.checked ? 'text-white' : 'text-gray-500 group-hover:text-gray-300'}`}>
                    {service.name}
                  </span>
                </div>
              ))}
              ...
              {directorServices.map((service) => (
                <div key={service.id} className="flex items-center gap-4 group cursor-pointer">
                  ...
                  <div className="w-10 h-10 flex-shrink-0">
                    ...
                    {/* Note: CSS filter used above to shift pink icon to blue if you use placeholder. Remove filter if real icon used. */}
                  </div>
